...
static bool propagate(void) {
  ...
  for (auto c : clauses) {
    // Return 'false' if propagation detects an empty clause
    if (c->size == 0)
      return false;
  }
  // Loop while there are unpropagated literals on the trail
  while (propagated < trail.size()) {
    int lit = trail[propagated];
    // Propagate the next unpropagated literal on the trail
    propagated++;
    // Increase propagations
    propagations++;
    // Iterate over all clauses
    for (auto c : clauses) {
      // Check whether the clause is satisfied, falsified, or forcing
      if (!satisfied(c)) {
        // Iterate over all literals in the clause
        for (auto l : *c) {
          // Check if -lit is in the clause
          if (l == -lit) {
            // Assign literal if the clause is forcing
            if (falsified(c)) {
              conflicts++;
              return false;
            } else {
  ...
  // Satisfied if all literals propagated without finding a falsified clause
  return true;
}
...
static void unassign(int lit) {
  ...
  // Reset 'values[lit]' & 'values[-lit]'
  values[lit] = 0;
  values[-lit] = 0;
  // Reset 'levels[abs(lit)]'
  levels[abs(lit)] = 0;
  // Pop literal from trail.
  trail.pop_back();
}
...
static void backtrack() {
  ...
  int height_prev = control.back();
  control.pop_back();

  // Unassign all literals starting from previous to current trail height.
  for (int i = trail.size() - 1; i >= height_prev; i--) {
    unassign(trail[i]);
  }

  // Set 'propagated' to trail height.
  propagated = height_prev;
...
int main(int argc, char **argv) {
  ...
  line();
  message("Copyright (c) 2022-2023, Onur Altinordu");
